...
// Kraków + MLD: the KMK network (ZTP Kraków — buses and trams, two feeds of one
// operator), the Wieliczka commune buses (WST, via KiedyPrzyjedzie — see
// pipeline/kp-wst-gtfs.py) and, on the same sheet, the Małopolskie Linie Dowozowe — the
// Koleje Małopolskie feeder buses A1…A71 that run from the railway stations
// across the whole voivodeship (feed from files.girlc.at, CC0; no shapes, no
// direction_id — the stop sequence is the observation and the headsign the
// direction key). Both bus operators pour into one bus cfg; their keys never
// collide (KMK numbers, MLD "A" + number), and the natural sort puts the
// A-lines after every KMK number in every list — street rows, badges, panel.
// The road graph is the Kraków extract plus a regional extract of Małopolska
// (the MLD corridors; see download.sh), merged at load.
...
// TWO operators, two cfgs: the bus cfg takes the KMK bus feed AND the MLD feed
// (routeTypes 3; the MLD feed's "ZKA …" routes are rail-replacement buses —
// skipped like Budapest's pótló); the tram cfg takes the KMK tram feed, whose
// line list is explicit (package.json) because that feed also carries
// replacement buses under their own numbers.
const MODES = [{
  mode: 'bus', label: 'buses',
  osmFiles: ['data/osm/krakow.json', 'data/osm/malopolska.json'],
  ...
  feeds: [
    ...
    { tag: 'mld', dir: 'data/gtfs-mld', mapKey: (sn) => sn, routeTypes: ['3'],
      skipRoute: (r) => /^ZKA\b/i.test((r.route_short_name || '').trim()) },
    // Wielicka Spółka Transportowa (Wieliczka commune buses) via KiedyPrzyjedzie
    // — pipeline/kp-wst-gtfs.py builds the GTFS, no shapes, no direction_id
    { tag: 'wst', dir: 'data/gtfs-wst', mapKey: (sn) => sn, routeTypes: ['3'] },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
// The rail network of Koleje Małopolskie gets the rail-trunk treatment the
// family gives a metro or an S-Bahn: a wide ribbon, station discs and names
// that never fade. It is the third thing
// Koleje Małopolskie put on this sheet, beside the MLD buses.
// Everything Koleje Małopolskie run on rails gets the rail-trunk treatment
// (wide ribbon, station discs, names that never fade): the three numbered SKA
// lines and the trains the operator brands by NAME instead of by number —
// Dunajec, Hubal, Luxtorpeda — plus the unnumbered regional service the feed
// files as "KML" (Nowy Sącz – Gorlice – Jasło).
const KM_RAIL = /^(SKA\d|KML$|Dunajec|Hubal|Luxtorpeda|Jaskółka|Włóczykij|Mościcki)/;
const isRailTrunk = (l) => KM_RAIL.test(l);
...
if (tramAll || skaSel.length) MODES.push({
  ...
  // is the family's exception to the mode-colour rule). Drawn WHOLE — which on
  // a Małopolska sheet is what the trains actually are.
  mode: 'tram', label: 'Koleje Małopolskie', osmFiles: ['data/osm/krakow-rail.json'],
  ...
  feeds: [
    ...
    // Every rail route of the operator, not only the numbered ones: route_type
    // 2 takes SKA1–SKA3 and leaves the ZKA replacement buses (type 3) out by
    // itself. A route with no short name is one of the named trains, and the
    // feed writes those in capitals — printed the way the timetable does.
    // Every rail route of the operator, not only the numbered ones: route_type
    // 2 takes SKA1–SKA3 and leaves the ZKA replacement buses (type 3) out by
    // itself. A route with no short name is one of the named trains, and the
    // feed writes those in capitals — printed the way the timetable does.
    // skipRoute: the Beliansky Express is the one Koleje Małopolskie train
    // that runs ABROAD (Muszyna – Plavec, Slovakia); this sheet is Małopolska,
    // so it stays out.
    { tag: 'ska', dir: 'data/gtfs-ska', routeTypes: ['2'],
      skipRoute: (r) => /BELIANSKY/i.test(`${r.route_short_name || ''} ${r.route_long_name || ''} ${r.route_id || ''}`),
      mapKey: (sn, r) => {
        const n = (sn || r.route_long_name || r.route_id || '').trim();
        if (!n) return null;
        if (/^SKA\d/.test(n) || n === 'KML') return n;
        return n.split(/\s+/).map((w) => w[0] + w.slice(1).toLowerCase()).join(' ');
      } },
  ],
});
...
await (await import('./night.mjs')).nightPass(outDir, /^(6\d|[69]\d\d)$/, { sort: true });
...
await (await import('./railrows.mjs')).railRowPass(outDir, /^(6\d|[69]\d\d)$/, { log });
